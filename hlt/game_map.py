    ...
    def _update(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                hlt.control.all_halite_amounts.append(self[Position(x, y)].halite_amount)
                hlt.control.unsafe_locs[Position(x, y)] = False
                hlt.control.is_target[Position(x, y)] = False
                if self[Position(x, y)].halite_amount >= hlt.control.minimum_target:
                    hlt.control.num_eligible_spaces += 1
