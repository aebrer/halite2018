...
class Game:
    ...
    def update_frame(self):
        ...
        dos = self.me.get_dropoffs()
        dos_pos = [do.position for do in dos]
        dos_pos.append(self.me.shipyard.position)

        # Mark cells with ships as unsafe for navigation
        for player in self.players.values():
            if player.id == self.me.id:
                for ship in player.get_ships():
                        can_move = ship.halite_amount >= self.game_map[ship.position].halite_amount / constants.MOVE_COST_RATIO
                        if not can_move:
                            self.game_map[ship.position].mark_unsafe(ship)
            else:
                for ship in player.get_ships():
                    not_on_do = True
                    if ship.position in dos_pos:
                        not_on_do = False
                    if not_on_do:
                        self.game_map[ship.position].mark_unsafe(ship)
